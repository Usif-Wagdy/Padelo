...
const PadelBooking = () => {
    ...
    const isAvailable = () => {
      for (let i = 0; i < parseInt(duration); i++) {
        ...
        const slotAvailable = slots.some(slot => 
          slot.number === currentSlot && slot.day === day
        );
  ...
  const filteredSlotsReserved = slotsReserved.filter((slot) => slot.day === day);
  ...
  return (
    ...
    <div className="padel-page-container" >
      ...
      <section className="padel-booking-section">
        ...
        <form className="padel-booking-form" onSubmit={(e) => {
          e.preventDefault(); 
          bookduration();
        }}>
          ...
          <p> 
            Price: <strong>{courtData ? courtData.price : "Loading..."} LE</strong>
          </p>
      ...
      <section className="padel-slots-section">
        <h3 style={{ textAlign: "center", color: "green", fontSize: "20px" }}>Available Slots</h3>
        <div className="padel-slots-grid">
          {filteredSlots.length > 0 ? (
            filteredSlots.map((slot) => (
              <div key={slot.number} className="padel-slot padel-slot-available">
      ...
      <section className="padel-slots-section">
        <h3 style={{ textAlign: "center", color: "red", fontSize: "20px" }}>Reserved Slots</h3>
        <div className="padel-slots-grid">
          {filteredSlotsReserved.length > 0 ? (
            filteredSlotsReserved.map((slot) => (
              <div key={slot.number} className="padel-slot padel-slot-reserved">
                {` ${slot.number} clock (${slot.day})`}
              </div>
            ))
          ) : (
